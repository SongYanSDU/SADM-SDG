...
def cov_loss(x1, x2, x3, x12, x13, x23):
    loss = tf.reduce_mean(covariance_loss(x1, x12) + covariance_loss(x1, x13) +
                          covariance_loss(x2, x12) + covariance_loss(x2, x23) +
                          covariance_loss(x3, x13) + covariance_loss(x3, x23) +
                          covariance_loss(x12, x13) + covariance_loss(x13, x23) + covariance_loss(x12, x23))
    # loss += tf.reduce_mean(covariance_loss(x12, x13) + covariance_loss(x13, x23) + covariance_loss(x12, x23))
    return loss
...
class FaultDiagnosisModel:
    def __init__(self, signal_length=4096, num_classes=5):
        ...
        self.conv2_1 = Conv1D(32, 9, activation='relu', padding='same', name='x1_conv2')
        self.conv2_2 = Conv1D(32, 3, activation='relu', padding='same')
        # self.conv2_3 = Conv1D(32, 5, activation='relu', padding='same')
        self.pool2_1 = MaxPooling1D(8)
        self.pool2_2 = MaxPooling1D(8)
        self.pool2_3 = MaxPooling1D(8)
        ...
        self.pool3_1 = MaxPooling1D(2)
        self.pool3_2 = MaxPooling1D(2)
        self.pool3_3 = MaxPooling1D(2)
    ...
    def build_model(self):
        ...
        x2 = self.conv2_2(diffusion_input_1)
        x2 = self.pool2_2(x2)
        x2 = self.conv3_2(x2)
        x2 = self.pool3_2(x2)

        diffusion_input_2 = Input(shape=(128, 32))
        # x3 = self.conv2_3(diffusion_input_2)
        x3 = self.pool2_3(diffusion_input_2)
        x3 = self.conv3_3(x3)
        x3 = self.pool3_3(x3)
...
# 定义训练和测试对
data_pairs = [(data1, label1, data3, label3),  # W20 → W30 0.5
              (data2, label2, data3, label3),  # W25 → W30 0.5
              (data2, label2, data4, label4),  # W25 → W35 0.5
              (data3, label3, data5, label5),  # W30 → W40 0.5
              (data3, label3, data6, label6),  # W30 → W45 0.5
              (data4, label4, data6, label6),  # W35 → W45 0.05
              (data4, label4, data7, label7),  # W35 → W50 0.5
              (data5, label5, data7, label7)]  # W40 → W55]

for i, (tr_data, tr_label, tt_data, tt_label) in enumerate(data_pairs):
    num = 5000
    train_data = tr_data[:num]
    train_label = tr_label[:num]
    test_data = tt_data[:num]
    test_label = tt_label[:num]
    # 数据标准化
    ...
    print("epoch: {:.4f}, Test loss: {:.4f}, Test Accuracy: {:.4f}".format(epoch, eval[0], eval[1]))
    print(f"Finished Training and Testing Pair {i + 1}\n")
    # 保存扩散模型和故障诊断模型
    path = '.\\pp\\'
    experiment_id = f"case{i + 1}"
    fea1, fea2 = diffusion_model.extract_encoder_features(x_test)
